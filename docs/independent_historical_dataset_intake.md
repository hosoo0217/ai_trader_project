...
- A read-only temporal-overlap diagnostic review completed on `2026-07-17` confirmed that the current frozen runner schedules entry windows every `5` candles while simulated outcome horizons can continue across later scheduled entries; `44` of `68` realized outcomes exited after the next scheduled entry. Strict and inclusive non-overlap selections produced materially different descriptive counterfactual results, so neither replaces the official reproducible failure or authorizes strategy, risk, or threshold changes. Dataset acceptance remains valid, paper progression remains blocked, and code freeze remains active. External review: `temporal_overlap_diagnostic_review_2026-07-17.md`, SHA-256 `D132986E0152A4756956E09EE0B671939F05BDF43FCBA006D0E368E803BE05D1`.
